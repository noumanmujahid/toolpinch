...
function tpWords(text){return (text||'').toLowerCase().replace(/[^a-z0-9\s+-]/g,' ').split(/\s+/).filter(w=>w.length>2)}
function tpTopKeywords(text,limit=30){const stop=new Set('the and for with from that this your you are have has was were will can job role our their they into about using use skills experience work team candidate candidates requirements required years plus must should ability able strong good excellent'.split(' '));const counts=new Map();tpWords(text).forEach(w=>{if(!stop.has(w))counts.set(w,(counts.get(w)||0)+1)});return [...counts.entries()].sort((a,b)=>b[1]-a[1]||a[0].localeCompare(b[0])).slice(0,limit)}
function atsResumeChecker(){const resume=document.getElementById('resumeText').value||'';const job=document.getElementById('jobText').value||'';const r=document.getElementById('result');if(resume.length<40||job.length<40){r.innerHTML='Paste both resume text and job description text.';return}const jobKeys=tpTopKeywords(job,35).map(x=>x[0]);const resumeWords=new Set(tpWords(resume));const found=jobKeys.filter(k=>resumeWords.has(k));const missing=jobKeys.filter(k=>!resumeWords.has(k));const score=Math.round(found.length/Math.max(1,jobKeys.length)*100);r.innerHTML='<strong>Estimated keyword match: '+score+'%</strong><small>This is a keyword check only, not a hiring prediction.</small><h3>Matched keywords</h3><pre>'+tpEscape(found.join(', ')||'None found')+'</pre><h3>Missing keywords to review</h3><pre>'+tpEscape(missing.slice(0,20).join(', ')||'None')+'</pre>'}
function resumeKeywordScanner(){const resume=document.getElementById('resumeText').value||'';const r=document.getElementById('result');if(resume.length<40){r.innerHTML='Paste resume text first.';return}const keys=tpTopKeywords(resume,40);r.innerHTML='<strong>Top resume keywords</strong><pre>'+tpEscape(keys.map(([w,c])=>w+' - '+c).join('\n'))+'</pre><small>Use this to spot repeated terms, weak wording and missing role language.</small>'}
function jobKeywordExtractor(){const job=document.getElementById('jobText').value||'';const r=document.getElementById('result');if(job.length<40){r.innerHTML='Paste a job description first.';return}const keys=tpTopKeywords(job,45);r.innerHTML='<strong>Important job description keywords</strong><pre>'+tpEscape(keys.map(([w,c])=>w+' - '+c).join('\n'))+'</pre><small>Group these into skills, tools, responsibilities and outcomes before editing your resume.</small>'}
function salaryHourlyCalc(){const salary=Number(document.getElementById('salaryAnnual').value);const hours=Number(document.getElementById('hoursWeek').value)||40;const weeks=Number(document.getElementById('weeksYear').value)||52;const r=document.getElementById('result');if(!salary||salary<0||hours<=0||weeks<=0){r.innerHTML='Enter a valid salary, weekly hours and paid weeks.';return}const hourly=salary/(hours*weeks);r.innerHTML='<strong>Estimated hourly rate: '+hourly.toFixed(2)+'</strong><small>Weekly: '+(salary/weeks).toFixed(2)+' · Monthly: '+(salary/12).toFixed(2)+' · Daily at 5 days/week: '+(salary/(weeks*5)).toFixed(2)+'</small>'}
function noticePeriodCalc(){const raw=document.getElementById('noticeStart').value;const days=Number(document.getElementById('noticeDays').value);const r=document.getElementById('result');if(!raw||days<0){r.innerHTML='Select a start date and notice period days.';return}const date=new Date(raw+'T00:00:00');date.setDate(date.getDate()+days);r.innerHTML='<strong>Estimated final day: '+date.toLocaleDateString(undefined,{weekday:'long',year:'numeric',month:'long',day:'numeric'})+'</strong><small>Check your contract and local rules before acting on this estimate.</small>'}
function jobTrackerLoad(){try{return JSON.parse(localStorage.getItem('tp_jobs')||'[]')}catch(e){return[]}}
function jobTrackerSave(items){localStorage.setItem('tp_jobs',JSON.stringify(items.slice(0,50)))}
function jobTrackerRender(){const items=jobTrackerLoad();const r=document.getElementById('result');r.innerHTML=items.length?'<div class="table-scroll"><table class="sports-table"><thead><tr><th>Date</th><th>Company</th><th>Role</th><th>Status</th></tr></thead><tbody>'+items.map(i=>'<tr><td>'+tpEscape(i.date||'')+'</td><td>'+tpEscape(i.company)+'</td><td>'+tpEscape(i.role)+'</td><td>'+tpEscape(i.status)+'</td></tr>').join('')+'</tbody></table></div><small>Saved only in this browser.</small>':'No applications saved yet.'}
function jobTrackerAdd(){const company=(document.getElementById('jobCompany').value||'').trim();const role=(document.getElementById('jobRole').value||'').trim();const status=document.getElementById('jobStatus').value;const date=document.getElementById('jobDate').value||new Date().toISOString().slice(0,10);if(!company||!role){result.innerHTML='Enter company and role.';return}const items=jobTrackerLoad();items.unshift({company,role,status,date});jobTrackerSave(items);jobTrackerRender()}
function jobTrackerClear(){localStorage.removeItem('tp_jobs');jobTrackerRender()}
function cvFileNameGenerator(){const name=(document.getElementById('cvName').value||'').trim();const role=(document.getElementById('cvRole').value||'').trim();const company=(document.getElementById('cvCompany').value||'').trim();if(!name||!role){result.innerHTML='Enter your name and target role.';return}const clean=s=>s.toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-+|-+$/g,'');const base=[clean(name),clean(role),company?clean(company):'',new Date().getFullYear()].filter(Boolean).join('-');result.innerHTML='<pre>'+tpEscape([base+'.pdf',base+'-resume.pdf',base+'-cv.pdf'].join('\n'))+'</pre><small>Use clean file names without extra symbols or version clutter.</small>'}
document.addEventListener('DOMContentLoaded',()=>{if(location.pathname.includes('/job-application-tracker'))jobTrackerRender()});
